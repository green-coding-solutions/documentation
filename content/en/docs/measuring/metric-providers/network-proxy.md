---
title: "Network Proxy"
description: "Documentation for the network proxy for the Green Metrics Tool"
lead: ""
...
It is worth highlight that this proxy only works with **external** resources. It does not report on inter docker network
communication. It also currently does not record the amount of data transferred.
...
measuring. Once the `tinyproxy` has loaded all docker containers are started with the `httpProxy` and the `httsProxy`
...
docker demon as this might interfere with other docker containers running on the same demon but are not part
of our benchmark.
...
system. Please refer to the configuration part to see how this can be mitigated.

### Configuration

The network proxy does not need any configuration by default. It is however possible to set the host ip if our logic
does not resolve a correct ip on Linux.
...
The host ip needs to be reachable from inside of the container and the proxy needs to be accessible on port 8889.
